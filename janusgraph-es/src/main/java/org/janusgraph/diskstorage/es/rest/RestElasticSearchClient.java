...
public class RestElasticSearchClient implements ElasticSearchClient {
    ...
    public IndexMapping getMapping(String indexName, String typeName) throws IOException {
        ...
        if(majorVersion.getValue() > 6) {
            return getTypelessMapping(indexName);
        } else {
            return getTypedMapping(indexName, typeName);
        }
    }

    /**
     * Is used for ElasticSearch before 7
     */
    private IndexMapping getTypedMapping(String indexName, String typeName) throws IOException {
    ...
    /**
     * Is used for ElasticSearch 7 or more
     */
    private IndexMapping getTypelessMapping(String indexName) throws IOException {

        Request request = new Request(REQUEST_TYPE_GET, REQUEST_SEPARATOR + indexName + REQUEST_SEPARATOR + "_mapping");

        try (final InputStream inputStream = performRequest(request, null).getEntity().getContent()) {
            final Map<String, IndexMappings> settings = mapper.readValue(inputStream, new TypeReference<Map<String, IndexMappings>>() {});
            return settings != null ? settings.get(indexName).getMappings().values().iterator().next() : null;
        } catch (final JsonParseException | JsonMappingException | ResponseException e) {
            log.info("Error when we try to get ES mapping", e);
            return null;
        }
